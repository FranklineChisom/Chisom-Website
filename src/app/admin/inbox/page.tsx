...
export default function InboxManager() {
  ...
  const handleDelete = async () => {
      ...
      if (activeFolder === 'trash') {
          if (window.confirm('Delete permanently? This cannot be undone.')) {
              ...
              if (success) { showToast('Deleted permanently', 'success'); setSelectedId(null); }
          }
      } else {
          const success = await moveToTrash(selectedItem.id, selectedItem.type);
          if (success) { showToast('Moved to Trash', 'success'); setSelectedId(null); }
      }
  };
  ...
  const handleRestore = async () => {
      ...
      if (success) { showToast('Restored', 'success'); setSelectedId(null); }
  };
  ...
  const renderViewAttachments = (raw: ContactMessage | SentEmail) => {
      if (!raw.attachments || raw.attachments.length === 0) return null;
      return (
          <div className="mt-6 pt-6 border-t border-slate-100">
              <h5 className="text-xs font-bold text-slate-500 uppercase tracking-wider mb-3 flex items-center gap-2">
                  <Paperclip size={14} /> {raw.attachments.length} Attachment{raw.attachments.length !== 1 ? 's' : ''}
              </h5>
              <div className="flex flex-wrap gap-3">
                  {raw.attachments.map((att, i) => (
                      <a 
                        key={i} 
                        href={att} 
                        target="_blank" 
  ...
  return (
    <div className="flex h-[calc(100vh-100px)] bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden animate-in fade-in duration-500 relative">
      ...
      {showMediaLibrary && (
        <MediaLibrary onSelect={handleAttach} onClose={() => setShowMediaLibrary(false)} />
      )}

      {/* Sidebar */}
      <div className="w-64 bg-slate-50 border-r border-slate-200 flex flex-col hidden md:flex">
        <div className="p-4">
            <button onClick={() => handleCompose()} className="w-full bg-primary text-white py-3 rounded-lg flex items-center justify-center gap-2 font-medium hover:bg-slate-800 transition-colors shadow-sm">
                <Edit3 size={18} /> Compose
            </button>
        </div>
        <nav className="flex-1 px-3 space-y-1">
            <button onClick={() => { setActiveFolder('inbox'); setSelectedId(null); setIsComposing(false); }} className={`w-full flex items-center justify-between px-4 py-2.5 rounded-md text-sm font-medium transition-colors ${activeFolder === 'inbox' ? 'bg-white text-primary shadow-sm ring-1 ring-slate-100' : 'text-slate-600 hover:bg-slate-200/50'}`}>
                <div className="flex items-center gap-3"><Inbox size={18} /> Inbox</div>
                {messages.filter(m => !m.read && !m.deleted_at).length > 0 && <span className="bg-primary text-white text-[10px] font-bold py-0.5 px-2 rounded-full">{messages.filter(m => !m.read && !m.deleted_at).length}</span>}
            </button>
            <button onClick={() => { setActiveFolder('sent'); setSelectedId(null); setIsComposing(false); }} className={`w-full flex items-center justify-between px-4 py-2.5 rounded-md text-sm font-medium transition-colors ${activeFolder === 'sent' ? 'bg-white text-primary shadow-sm ring-1 ring-slate-100' : 'text-slate-600 hover:bg-slate-200/50'}`}>
                <div className="flex items-center gap-3"><Send size={18} /> Sent</div>
            </button>
            <button onClick={() => { setActiveFolder('drafts'); setSelectedId(null); setIsComposing(false); }} className={`w-full flex items-center justify-between px-4 py-2.5 rounded-md text-sm font-medium transition-colors ${activeFolder === 'drafts' ? 'bg-white text-primary shadow-sm ring-1 ring-slate-100' : 'text-slate-600 hover:bg-slate-200/50'}`}>
      ...
      <div className="w-80 border-r border-slate-200 flex flex-col bg-white">
        ...
        <div className="flex-1 overflow-y-auto">
            {listItems.length === 0 ? <div className="p-12 text-center text-slate-400 text-sm flex flex-col items-center"><Inbox size={32} className="mb-2 opacity-20" /><span>No messages</span></div> : listItems.map(item => (
                <div key={item.id} onClick={() => handleSelectItem(item)} className={`p-4 border-b border-slate-100 cursor-pointer transition-all hover:bg-slate-50 ${selectedId === item.id ? 'bg-blue-50/50 border-l-4 border-l-primary' : 'border-l-4 border-l-transparent'} ${!item.read ? 'bg-slate-50' : ''}`}>
        ...
        {isComposing ? (
            <div className="flex flex-col h-full animate-in fade-in zoom-in-95 duration-200">
                ...
                <div className="p-6 flex-1 flex flex-col gap-4 overflow-y-auto">
                    ...
                    <textarea value={composeData.body} onChange={e => setComposeData({...composeData, body: e.target.value})} className="flex-1 resize-none mt-4 p-4 border border-slate-200 rounded-md focus:border-primary focus:ring-1 focus:ring-primary focus:outline-none text-sm leading-relaxed transition-shadow" placeholder="Type your message here..." />
                ...
                <div className="p-4 border-t border-slate-100 bg-slate-50 flex justify-between items-center">
                    <div className="flex gap-2 text-slate-400">
                        <button onClick={() => setShowMediaLibrary(true)} className="p-2 hover:bg-slate-200 rounded-full transition-colors text-primary" title="Attach File"><Paperclip size={18} /></button>
                    </div>
                    <button onClick={handleSend} disabled={isSending} className="bg-primary text-white px-8 py-2.5 rounded-md font-medium hover:bg-slate-800 transition-all flex items-center gap-2 disabled:opacity-70 shadow-sm hover:shadow-md transform active:scale-95">{isSending ? <Loader2 size={18} className="animate-spin" /> : <Send size={18} />} {isSending ? 'Sending...' : 'Send Message'}</button>
                </div>
            </div>
        ) : selectedItem ? (
            <div className="flex flex-col h-full animate-in fade-in duration-300">
                <div className="px-8 py-6 border-b border-slate-100 flex justify-between items-start bg-slate-50/30">
                    ...
                    <div className="flex gap-2">
                        {activeFolder === 'inbox' && (
                            <>
                                <button onClick={handleMarkUnread} className="p-2 text-slate-500 hover:text-primary hover:bg-slate-100 rounded-md transition-colors" title="Mark as Unread"><MailOpen size={20} /></button>
                        ...
                        {activeFolder === 'trash' ? <button onClick={handleRestore} className="p-2 text-slate-500 hover:text-green-600 hover:bg-green-50 rounded-md transition-colors" title="Restore"><RotateCcw size={20} /></button> : null}
                        <button onClick={handleDelete} className="p-2 text-slate-400 hover:text-red-600 hover:bg-red-50 rounded-md transition-colors" title={activeFolder === 'trash' ? "Delete Permanently" : "Move to Trash"}><Trash2 size={20} /></button>
                    </div>
                </div>
                <div className="flex-1 p-8 overflow-y-auto">
                    ...
                    {/* Render Attachments */}
                    {(selectedItem.type === 'message' || selectedItem.type === 'sent') && 
                      renderViewAttachments(selectedItem.raw as ContactMessage | SentEmail)
                    }
                </div>
            </div>
        ) : (
            <div className="flex-1 flex flex-col items-center justify-center text-slate-300 bg-slate-50/30">
                <div className="w-16 h-16 bg-slate-100 rounded-full flex items-center justify-center mb-4"><Mail size={32} className="text-slate-400" /></div>

    ...
    <div className="flex h-[calc(100vh-100px)] bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden relative">
      ...
      <div className={`absolute md:relative z-30 w-64 bg-slate-50 border-r border-slate-200 flex flex-col h-full transition-transform duration-300 transform ${mobileMenuOpen ? 'translate-x-0' : '-translate-x-full'} md:translate-x-0`}>
        <div className="p-4 flex items-center justify-between md:block">
            ...
            <button onClick={() => setMobileMenuOpen(false)} className="md:hidden ml-2 text-slate-500">
                <X size={24} />
            </button>
        </div>
        ...
        <div className="flex-1 overflow-y-auto">
            {listItems.length === 0 ? <div className="p-12 text-center text-slate-400 text-sm flex flex-col items-center"><Inbox size={32} className="mb-2 opacity-20" /><span>No messages</span></div> : listItems.map(item => (
                <div key={item.id} onClick={() => handleSelectItem(item)} className={`group relative p-4 border-b border-slate-100 cursor-pointer transition-all hover:bg-slate-50 ${selectedId === item.id ? 'bg-blue-50/50 border-l-4 border-l-primary' : 'border-l-4 border-l-transparent'} ${!item.read ? 'bg-slate-50' : ''}`}>
                    <div className="absolute left-2 top-4 opacity-0 group-hover:opacity-100 transition-opacity z-10 block md:hidden md:block" onClick={(e) => e.stopPropagation()}>
      ...
      <div className={`flex-1 flex flex-col bg-white min-w-0 ${!selectedId && !isComposing ? 'hidden md:flex' : 'flex'}`}>
        {isComposing ? (
            <div className="flex flex-col h-full animate-in fade-in zoom-in-95 duration-200">
                <div className="px-4 md:px-6 py-4 border-b border-slate-100 flex justify-between items-center bg-slate-50/30">
                    <div className="flex items-center gap-2">
                        <button className="md:hidden mr-2 text-slate-500" onClick={() => setIsComposing(false)}>
                            <ChevronLeft size={20} />
                        </button>
                        <h3 className="font-serif text-lg text-slate-800">New Message</h3>
                    </div>
                    <div className="flex gap-2">
                        <button onClick={handleSaveDraft} className="px-3 md:px-4 py-2 text-slate-600 hover:bg-slate-100 rounded-md text-xs md:text-sm font-medium transition-colors">Save Draft</button>
                        <button onClick={() => setIsComposing(false)} className="p-2 text-slate-400 hover:text-slate-600 hover:bg-slate-100 rounded-full transition-colors hidden md:block"><X size={20} /></button>
                    </div>
                </div>
                <div className="p-4 md:p-6 flex-1 flex flex-col gap-4 overflow-y-auto">
                    <div className="flex flex-col md:flex-row md:items-center gap-2 md:gap-4">
                        <label className="w-16 text-sm font-medium text-slate-500 hidden md:block">To:</label>
                        <input type="email" value={composeData.to} onChange={e => setComposeData({...composeData, to: e.target.value})} className="flex-1 border-b border-slate-200 py-2 focus:border-primary focus:outline-none text-sm bg-transparent transition-colors" placeholder="Recipient (to...)" />
                    </div>
                    <div className="flex flex-col md:flex-row md:items-center gap-2 md:gap-4">
                        <label className="w-16 text-sm font-medium text-slate-500 hidden md:block">Subject:</label>
                        <input type="text" value={composeData.subject} onChange={e => setComposeData({...composeData, subject: e.target.value})} className="flex-1 border-b border-slate-200 py-2 focus:border-primary focus:outline-none text-sm font-medium bg-transparent transition-colors" placeholder="Subject" />
                    </div>
                ...
                <div className="p-4 border-t border-slate-100 bg-slate-50 flex justify-between items-center">
                    <button onClick={() => setShowMediaLibrary(true)} className="p-2 hover:bg-slate-200 rounded-full transition-colors text-slate-500 flex items-center gap-2 text-sm" title="Attach File">
                        <Paperclip size={18} /> <span className="hidden md:inline">Attach</span>
                    </button>
                    <button 
                        onClick={handleSend} 
                        disabled={isSending} 
                        className="bg-primary text-white px-6 md:px-8 py-2.5 rounded-md font-medium hover:bg-slate-800 transition-all flex items-center gap-2 disabled:opacity-70 shadow-sm hover:shadow-md transform active:scale-95 text-sm md:text-base"
                    >
            ...
            <div className="flex flex-col h-full animate-in fade-in duration-300">
                {/* Message Header */}
                <div className="px-4 md:px-8 py-6 border-b border-slate-100 bg-slate-50/30 sticky top-0 z-10 backdrop-blur-sm">
                    <div className="flex justify-between items-start mb-4">
                        <div className="flex items-center gap-2">
                            ...
                            <h2 className="text-lg md:text-2xl font-serif text-slate-900 leading-tight line-clamp-2">{selectedItem.subtitle}</h2>
                        </div>
                        <div className="flex gap-2">
                            {activeFolder === 'inbox' && (
                                <>
                                    ...
                                    <button onClick={() => handleCompose({ to: (selectedItem.raw as ContactMessage).email, subject: `Re: ${selectedItem.subtitle}` })} className="flex items-center gap-2 px-3 md:px-4 py-2 bg-white border border-slate-200 hover:border-primary hover:text-primary text-slate-700 rounded-md text-sm font-medium transition-all shadow-sm"><CornerUpLeft size={16} /> <span className="hidden md:inline">Reply</span></button>
                                </>
                            )}
                    ...
                    <div className="flex items-center gap-4">
                        <div className="w-10 h-10 bg-gradient-to-br from-slate-100 to-slate-200 rounded-full flex items-center justify-center text-slate-500 border border-slate-200 shadow-sm flex-shrink-0"><User size={20} /></div>
                        <div className="min-w-0">
                            <div className="font-bold text-slate-800 text-sm truncate">
                                {selectedItem.title}
                                {selectedItem.type === 'message' && <span className="text-slate-400 font-normal ml-1 hidden sm:inline">&lt;{(selectedItem.raw as ContactMessage).email}&gt;</span>}
                            </div>
                ...
                <div className="flex-1 p-4 md:p-8 overflow-y-auto">
                    ...
                    {/* Optional chaining check for attachments */}
                            ...
                            <div className="flex flex-wrap gap-3">
                                {selectedItem.raw.attachments.map((att, i) => (
                                    <a 
                                      ...
                                      className="flex items-center gap-3 p-3 bg-slate-50 border border-slate-200 rounded-md hover:bg-slate-100 hover:border-primary transition-colors group max-w-full"
                                    >
                                        <div className="w-8 h-8 bg-white rounded-full flex items-center justify-center border border-slate-100 text-primary flex-shrink-0">
                                            <File size={16} />
                                        </div>
            ...
            <div className="flex-1 flex flex-col items-center justify-center text-slate-300 bg-slate-50/30 p-6 text-center">
                ...
                <p className="text-xs mt-2 hidden md:block">Use <kbd className="font-mono bg-white px-1 border rounded">C</kbd> to compose</p>
            </div>
        )}
      </div>
    </div>
...
const FolderButton = ({ active, onClick, icon, label, count, variant = 'default', countColor = 'bg-primary text-white' }: any) => (
    <button onClick={onClick} className={`w-full flex items-center justify-between px-4 py-3 md:py-2.5 rounded-md text-sm font-medium transition-all duration-200 ${active 
